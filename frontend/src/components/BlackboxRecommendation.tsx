...
const BlackboxRecommendation: React.FC = () => {
  ...
  return (
    <Card
      title={<Space><span style={{ width:28,height:28,borderRadius:8,background:'#FFF7ED',display:'inline-flex',alignItems:'center',justifyContent:'center',border:'1px solid #FED7AA' }}><ThunderboltOutlined style={{ color:'#D97706',fontSize:14 }} /></span><span>智能黑箱建议</span></Space>}
    ...
    >
      ...
      <Space direction="vertical" size="large" style={{ width:'100%' }}>
        <div>
          <div style={{ fontWeight:600,marginBottom:12,fontSize:14,color:'#292524' }}>📋 相似成功案例（{blackbox_recommendation.similar_cases_count}个）</div>
        ...
        <div>
          <div style={{ fontWeight:600,marginBottom:12,fontSize:14,color:'#292524' }}>🗺️ 推荐执行路径</div>
        ...
        <div>
          <div style={{ fontWeight:600,marginBottom:12,fontSize:14,color:'#292524' }}>置信度: {(blackbox_recommendation.confidence_score * 100).toFixed(0)}%</div>
          <Progress percent={Math.round(blackbox_recommendation.confidence_score * 100)} status="active" strokeColor={{ '0%': '#D97706', '100%': '#059669' }} />
        </div>
